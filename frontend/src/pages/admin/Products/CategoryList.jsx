...
// 트리 아이템 컴포넌트
// TreeItem - 버튼 제거하고 선택 UI만 남기기
const SortableTreeItem = ({ item, selectedId, onSelect, onDelete, depth = 0 }) => {
    ...
    return (
        <div ref={setNodeRef} style={style}>
            <div
                className={`flex items-center gap-1 py-1.5 rounded group transition-colors cursor-pointer ${
                    isSelected
            ...
            >
                ...
                <button
                    ...
                    className="w-4 h-4 flex items-center justify-center text-gray-400 shrink-0 text-xs"
                >
                    {hasChildren ? (open ? '▼' : '▶') : '•'}
                </button>
                {/* 카테고리명 */}
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${
                    item.is_active == 1 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'
                }`}>
                ...
                <button
                    ...
                    className="hidden group-hover:flex w-5 h-5 items-center justify-center text-xs bg-red-100 text-red-500 rounded hover:bg-red-200 shrink-0"
                >
                    ✕
                </button>
            </div>
...
// 같은 부모 내에서 순서 변경
    const reorderChildren = (items, parentId, activeId, overId) => {
        return items.map(item => {
            ...
            if (item.children?.length) {
                return { ...item, children: reorderChildren(item.children, parentId, activeId, overId) }
            }
            return item
        })
    }
    ...
    const handleDragEnd = async ({ active, over }) => {
        ...
        console.log('activeParent:', activeItem?.parentId)
        console.log('overParent:', overItem?.parentId)

        // parentId 정규화 (null, undefined, "0", 0 모두 null로)
        const normalizeParentId = (pid) => (!pid || pid == 0) ? null : pid
        const activeParent = normalizeParentId(activeItem?.parentId)
        const overParent   = normalizeParentId(overItem?.parentId)
        ...
        if (activeParent === null) {
            ...
            newTree = arrayMove([...tree], oldIndex, newIndex)
        } else {
            newTree = reorderChildren(tree, activeParent, active.id, over.id)
        }

        setTree(newTree)

        // 새 순서로 API 호출
        const flatNew  = flattenTree(newTree)
        const siblings = activeParent === null
            ? newTree
            : flatNew.filter(i => normalizeParentId(i.parentId) == activeParent)

        try {
            await api.post('/admin/products/categories/reorder', {
        ...
        <div>
            ...
            <div className="flex gap-4 items-start">
                ...
                <div className="w-72 shrink-0 bg-white rounded-lg shadow overflow-hidden">
                    ...
                    {/* 하위 카테고리 추가 버튼 - 트리 위에 노출 */}
                    {selectedId !== null && !isCreate && (
                    ...
                    {/* 선택된 카테고리명 찾기 */}
                                    {(() => {
                                        const findName = (items) => {
                                            for (const item of items) {
                                                if (item.id === selectedId || item.id == selectedId) return item.name
                                                if (item.children?.length) {
                            ...
                            <button
                                onClick={() => handleAdd(selectedId)}
                                className="w-full bg-blue-500 text-white text-xs px-3 py-1.5 rounded hover:bg-blue-600"
                            >
                                + 하위 카테고리 추가
                            </button>
                        </div>
                    ...
                    <div className="py-2 max-h-[calc(100vh-200px)] overflow-y-auto">
                        {treeLoading ? (
                            ...
                            >
                                ...
                                <DragOverlay>
                                    {activeId ? (
                                        <div className="bg-white shadow-lg rounded px-3 py-1.5 text-sm text-gray-700 border border-orange-300">
                                            {flattenTree(tree).find(i => i.id == activeId)?.name}
                                        </div>
                                    ) : null}
                                </DragOverlay>
                            </DndContext>
